...
- `POST /match/major-job`
  ...
  - `algorithm_version` 说明：
    ...
    - `v1.1-hybrid-emb`：轻量机器学习增强（预训练向量语义 + 语义覆盖融合 + IDF技能加权 + 约束惩罚；向量服务不可用时自动回退）
...
- `POST /match/major-job/batch`
  ...
  - 请求体（可选）：`algorithm_version`（默认 `v0.2-major-job`）、`persist`（默认 `false`）
  - `algorithm_version` 可选：`v0.2-major-job`、`v0.1-baseline`、`v1.0-hybrid-idf`、`v1.1-hybrid-emb`
...
- `POST /match/single`
  ...
  - `algorithm_version` 可选：`v0.1-baseline`、`v0.2-major-job`、`v1.0-hybrid-idf`、`v1.1-hybrid-emb`
