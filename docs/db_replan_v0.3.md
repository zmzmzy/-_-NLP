...
你当前看到的 19 张表并不是“错误设计”，而是**主流程 + 扩展流程 + 审计流程**混在一个库里，导致感知上“太多太乱”。

建议采用“分层收敛”而不是“推倒重建”：

- 核心层（答辩主线，必须保留）：专业数据、岗位数据、专业-岗位匹配
- 扩展层（可选能力，默认弱化）：学生就业追踪
- 支撑层（运维审计，按需保留）：导入日志、系统操作日志
...
- `major_skills`
- `job_skills`
- `major_job_match_results`（v0.3 新增，建议）
...
- M1：Core 主链路完全脱离学生表依赖。
- M2：前后端历史结果均使用 `major_job_match_results`。
- M3：扩展层可独立开关（隐藏/启用）。
